...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount)
	...
	if Player.Functions.RemoveItem(itemData.name, itemAmount, itemData.slot) then
		...
		TriggerClientEvent("QRCore:Notify", source, Lang:t("error.not_owned"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
		return
	end
end
...
RegisterNetEvent('inventory:server:combineItem', function(item, fromItem, toItem)
	local src = source
	local ply = exports['qr-core']:GetPlayer(src)
	...
	if not ply.state.inv_busy then
		if name and id then
			...
			if name == "stash" then
				if Stashes[id] then
					if Stashes[id].isOpen then
						local Target = exports['qr-core']:GetPlayer(Stashes[id].isOpen)
						if Target then
			...
			elseif name == "trunk" then
				if Trunks[id] then
					if Trunks[id].isOpen then
						local Target = exports['qr-core']:GetPlayer(Trunks[id].isOpen)
						if Target then
				...
				if (Trunks[id] and Trunks[id].isOpen) or (exports['qr-core']:SplitStr(id, "PLZI")[2] and Player.PlayerData.job.name ~= "police") then
					secondInv.name = "none-inv"
			...
			elseif name == "glovebox" then
				if Gloveboxes[id] then
					if Gloveboxes[id].isOpen then
						local Target = exports['qr-core']:GetPlayer(Gloveboxes[id].isOpen)
						if Target then
			...
			elseif name == "otherplayer" then
				local OtherPlayer = exports['qr-core']:GetPlayer(tonumber(id))
				if OtherPlayer then
					secondInv.name = "otherplayer-"..id
					secondInv.label = "Player-"..id
					secondInv.maxweight = exports['qr-core']:GetConfig().Player.MaxWeight
					secondInv.inventory = OtherPlayer.PlayerData.items
					if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
						secondInv.slots = exports['qr-core']:GetConfig().Player.MaxInvSlots
					else
						secondInv.slots = exports['qr-core']:GetConfig().Player.MaxInvSlots - 1
					end
					Wait(250)
				end
			else
				if Drops[id] then
					if Drops[id].isOpen then
						local Target = exports['qr-core']:GetPlayer(Drops[id].isOpen)
						if Target then
		...
		TriggerClientEvent('QRCore:Notify', src, 9, Lang:t("error.no_access"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
	end
end)
...
RegisterNetEvent('inventory:server:UseItemSlot', function(slot)
	...
	if itemData then
		...
		elseif itemData.useable then
			TriggerClientEvent("QRCore:Client:UseItem", src, itemData)
			TriggerClientEvent('inventory:client:ItemBox', src, itemInfo, "use")
		end
	end
end)
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if (fromInventory == "player" or fromInventory == "hotbar") and (exports['qr-core']:SplitStr(toInventory, "-")[1] == "itemshop" or toInventory == "crafting") then
		return
	end
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			elseif exports['qr-core']:SplitStr(toInventory, "-")[1] == "otherplayer" then
				local playerId = tonumber(exports['qr-core']:SplitStr(toInventory, "-")[2])
			...
			elseif exports['qr-core']:SplitStr(toInventory, "-")[1] == "trunk" then
				local plate = exports['qr-core']:SplitStr(toInventory, "-")[2]
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("qr-log:server:CreateLog", "trunk", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
					end
				else
					local itemInfo = sharedItems[fromItemData.name:lower()]
					TriggerEvent("qr-log:server:CreateLog", "trunk", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
				end
				local itemInfo = sharedItems[fromItemData.name:lower()]
				AddToTrunk(plate, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info)
			elseif exports['qr-core']:SplitStr(toInventory, "-")[1] == "glovebox" then
				local plate = exports['qr-core']:SplitStr(toInventory, "-")[2]
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("qr-log:server:CreateLog", "glovebox", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
					end
				else
					local itemInfo = sharedItems[fromItemData.name:lower()]
					TriggerEvent("qr-log:server:CreateLog", "glovebox", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
				end
				local itemInfo = sharedItems[fromItemData.name:lower()]
				AddToGlovebox(plate, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info)
			elseif exports['qr-core']:SplitStr(toInventory, "-")[1] == "stash" then
				local stashId = exports['qr-core']:SplitStr(toInventory, "-")[2]
			...
			-- elseif exports['qr-core']:SplitStr(toInventory, "-")[1] == "traphouse" then
				-- Traphouse
				-- local traphouseId = exports['qr-core']:SplitStr(toInventory, "-")[2]
				-- local toItemData = exports['qr-traphouse']:GetInventoryData(traphouseId, toSlot)
				-- local IsItemValid = exports['qr-traphouse']:CanItemBeSaled(fromItemData.name:lower())
				-- if IsItemValid then
					-- Player.Functions.RemoveItem(fromItemData.name, fromAmount, fromSlot)
					-- TriggerClientEvent("inventory:client:CheckWeapon", src, fromItemData.name)
					-- if toItemData ~= nil then
						...
						-- local toAmount = tonumber(toAmount) ~= nil and tonumber(toAmount) or toItemData.amount
						-- if toItemData.name ~= fromItemData.name then
							-- exports['qr-traphouse']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount)
							-- Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
							-- TriggerEvent("qr-log:server:CreateLog", "traphouse", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - traphouse: *" .. traphouseId .. "*")
						-- end
					-- else
						-- local itemInfo = sharedItems[fromItemData.name:lower()]
						-- TriggerEvent("qr-log:server:CreateLog", "traphouse", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - traphouse: *" .. traphouseId .. "*")
					-- end
					-- local itemInfo = sharedItems[fromItemData.name:lower()]
					-- exports['qr-traphouse']:AddHouseItem(traphouseId, toSlot, itemInfo["name"], fromAmount, fromItemData.info, src)
				-- else
					-- TriggerClientEvent('QRCore:Notify', src, 9, "You can\'t sell this item..", 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
				-- end
			else
				-- drop
			...
			TriggerClientEvent("QRCore:Notify", src, Lang:t("error.not_owned"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
		end
	elseif exports['qr-core']:SplitStr(fromInventory, "-")[1] == "otherplayer" then
		local playerId = tonumber(exports['qr-core']:SplitStr(fromInventory, "-")[2])
		local OtherPlayer = exports['qr-core']:GetPlayer(playerId)
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			TriggerClientEvent("QRCore:Notify", src, Lang:t("error.not_exist"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
		end
	elseif exports['qr-core']:SplitStr(fromInventory, "-")[1] == "trunk" then
		local plate = exports['qr-core']:SplitStr(fromInventory, "-")[2]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("qr-log:server:CreateLog", "trunk", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** plate: *" .. plate .. "*")
					else
						TriggerEvent("qr-log:server:CreateLog", "trunk", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from plate: *" .. plate .. "*")
					end
				else
					TriggerEvent("qr-log:server:CreateLog", "trunk", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** plate: *" .. plate .. "*")
				end
				Player.Functions.AddItem(fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = Trunks[plate].items[toSlot]
	...
	elseif exports['qr-core']:SplitStr(fromInventory, "-")[1] == "glovebox" then
		local plate = exports['qr-core']:SplitStr(fromInventory, "-")[2]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("qr-log:server:CreateLog", "glovebox", "Swapped", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src..")* swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** plate: *" .. plate .. "*")
					else
						TriggerEvent("qr-log:server:CreateLog", "glovebox", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from plate: *" .. plate .. "*")
					end
				else
					TriggerEvent("qr-log:server:CreateLog", "glovebox", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** plate: *" .. plate .. "*")
				end
				Player.Functions.AddItem(fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = Gloveboxes[plate].items[toSlot]
	...
	elseif exports['qr-core']:SplitStr(fromInventory, "-")[1] == "stash" then
		local stashId = exports['qr-core']:SplitStr(fromInventory, "-")[2]
	...
	-- elseif exports['qr-core']:SplitStr(fromInventory, "-")[1] == "traphouse" then
		-- local traphouseId = exports['qr-core']:SplitStr(fromInventory, "-")[2]
		-- local fromItemData = exports['qr-traphouse']:GetInventoryData(traphouseId, fromSlot)
		-- local fromAmount = tonumber(fromAmount) ~= nil and tonumber(fromAmount) or fromItemData.amount
		-- if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			-- local itemInfo = sharedItems[fromItemData.name:lower()]
			-- if toInventory == "player" or toInventory == "hotbar" then
				-- local toItemData = Player.Functions.GetItemBySlot(toSlot)
				-- exports['qr-traphouse']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], fromAmount)
				-- if toItemData ~= nil then
					-- local itemInfo = sharedItems[toItemData.name:lower()]
					-- local toAmount = tonumber(toAmount) ~= nil and tonumber(toAmount) or toItemData.amount
					-- if toItemData.name ~= fromItemData.name then
						-- Player.Functions.RemoveItem(toItemData.name, toAmount, toSlot)
						-- exports['qr-traphouse']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
						-- TriggerEvent("qr-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. traphouseId .. "*")
					-- else
						-- TriggerEvent("qr-log:server:CreateLog", "stash", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from stash: *" .. traphouseId .. "*")
					-- end
				-- else
					-- TriggerEvent("qr-log:server:CreateLog", "stash", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** stash: *" .. traphouseId .. "*")
				-- end
				-- Player.Functions.AddItem(fromItemData.name, fromAmount, toSlot, fromItemData.info)
			-- else
				-- local toItemData = exports['qr-traphouse']:GetInventoryData(traphouseId, toSlot)
				...
				-- if toItemData ~= nil then
					...
					-- if toItemData.name ~= fromItemData.name then
						-- local itemInfo = sharedItems[toItemData.name:lower()]
						-- exports['qr-traphouse']:RemoveHouseItem(traphouseId, toSlot, itemInfo["name"], toAmount)
						-- exports['qr-traphouse']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
					-- end
				-- end
				-- local itemInfo = sharedItems[fromItemData.name:lower()]
			...
			-- end
		-- else
			-- TriggerClientEvent("QRCore:Notify", src, "Item doesn't exist??", 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
		-- end
	elseif exports['qr-core']:SplitStr(fromInventory, "-")[1] == "itemshop" then
		local shopType = exports['qr-core']:SplitStr(fromInventory, "-")[2]
		local itemData = ShopItems[shopType].items[fromSlot]
		local itemInfo = sharedItems[itemData.name:lower()]
		local bankBalance = Player.PlayerData.money["bank"]
		local price = tonumber((itemData.price*fromAmount))

		if exports['qr-core']:SplitStr(shopType, "_")[1] == "Dealer" then
			if exports['qr-core']:SplitStr(itemData.name, "_")[1] == "weapon" then
				price = tonumber(itemData.price)
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					itemData.info.serie = tostring(exports['qr-core']:RandomInt(2) .. exports['qr-core']:RandomStr(3) .. exports['qr-core']:RandomInt(1) .. exports['qr-core']:RandomStr(2) .. exports['qr-core']:RandomInt(3) .. exports['qr-core']:RandomStr(4))
					...
					TriggerClientEvent('QRCore:Notify', src, 9, Lang:t("success.bought_item", {item = itemInfo["label"]}), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
					...
					TriggerClientEvent('QRCore:Notify', src, 9, Lang:t("error.no_cash"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
				end
			else
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					Player.Functions.AddItem(itemData.name, fromAmount, toSlot, itemData.info)
					TriggerClientEvent('qr-drugs:client:updateDealerItems', src, itemData, fromAmount)
					TriggerClientEvent('QRCore:Notify', src, 9, itemInfo["label"] .. " bought!", 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
					TriggerEvent("qr-log:server:CreateLog", "dealers", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. "  for $"..price)
				else
					TriggerClientEvent('QRCore:Notify', src, 9, Lang:t("error.no_cash"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
				end
			end
		elseif exports['qr-core']:SplitStr(shopType, "_")[1] == "Itemshop" then
			if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
                if exports['qr-core']:SplitStr(itemData.name, "_")[1] == "weapon" then
                    itemData.info.serie = tostring(exports['qr-core']:RandomInt(2) .. exports['qr-core']:RandomStr(3) .. exports['qr-core']:RandomInt(1) .. exports['qr-core']:RandomStr(2) .. exports['qr-core']:RandomInt(3) .. exports['qr-core']:RandomStr(4))
                end
				Player.Functions.AddItem(itemData.name, fromAmount, toSlot, itemData.info)
				TriggerClientEvent('qr-shops:client:UpdateShop', src, exports['qr-core']:SplitStr(shopType, "_")[2], itemData, fromAmount)
				TriggerClientEvent('QRCore:Notify', src, 9, Lang:t("success.bought_item", {item = itemInfo["label"]}), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
			...
			elseif bankBalance >= price then
				...
				TriggerClientEvent('QRCore:Notify', src, 9, "You don't have enough cash..", 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
			end
		else
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			TriggerClientEvent('QRCore:Notify', src, 9, Lang:t("error.missing_item"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
		end
...
RegisterServerEvent("inventory:server:GiveItem", function(target, inventory, item, amount)
    local src = source
    local Player = exports['qr-core']:GetPlayer(src)
    local OtherPlayer = exports['qr-core']:GetPlayer(tonumber(target))
    local dist = #(GetEntityCoords(GetPlayerPed(src))-GetEntityCoords(GetPlayerPed(target)))
	if Player == OtherPlayer then return TriggerClientEvent('QRCore:Notify', src, 9, Lang:t("error.yourself"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE') end
	if dist > 2 then return TriggerClientEvent('QRCore:Notify', src, 9, Lang:t("error.toofar"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE') end
	if amount <= item.amount then
		...
		if OtherPlayer.Functions.AddItem(item.name, amount, false, item.info) then
			...
			TriggerClientEvent('QRCore:Notify', target, Lang:t("success.recieved", {amount = amount, item = item.label, firstname = Player.PlayerData.charinfo.firstname, lastname = Player.PlayerData.charinfo.lastname}), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
			...
			TriggerClientEvent('QRCore:Notify', src, 9, Lang:t("success.gave", {amount = amount, item = item.label, firstname = OtherPlayer.PlayerData.charinfo.firstname, lastname = OtherPlayer.PlayerData.charinfo.lastname}), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
			TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
			TriggerClientEvent('qr-inventory:client:giveAnim', src)
			TriggerClientEvent('qr-inventory:client:giveAnim', target)
		else

			TriggerClientEvent('QRCore:Notify', src, 9, Lang:t("error.otherfull"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
			TriggerClientEvent('QRCore:Notify', target, Lang:t("error.invfull"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
			TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
			TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, false)
		end
	else
		TriggerClientEvent('QRCore:Notify', src, 9, Lang:t("error.not_enough"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
	end
end)

-- callback

exports['qr-core']:CreateCallback('qr-inventory:server:GetStashItems', function(source, cb, stashId)
	cb(GetStashItems(stashId))
end)

-- command

exports['qr-core']:AddCommand("resetinv", "Reset Inventory (Admin Only)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID of stash or license plate"}}, true, function(source, args)
	...
	if invType ~= nil and invId ~= nil then
		...
		elseif invType == "stash" then
			...
			TriggerClientEvent('QRCore:Notify', source, 9, Lang:t("error.invalid_type"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
		end
	else
		TriggerClientEvent('QRCore:Notify', source, 9, Lang:t("error.arguments"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
	end
end, "admin")

exports['qr-core']:AddCommand("rob", "Rob Player", {}, false, function(source, args)
	TriggerClientEvent("police:client:RobPlayer", source)
end)

exports['qr-core']:AddCommand("giveitem", "Give An Item (Admin Only)", {{name="id", help="Player ID"},{name="item", help="Name of the item (not a label)"}, {name="amount", help="Amount of items"}}, true, function(source, args)
		...
		if amount > 0 then
			if itemData then
				...
				elseif itemData["type"] == "weapon" then
					amount = 1
					info.serie = tostring(exports['qr-core']:RandomInt(2) .. exports['qr-core']:RandomStr(3) .. exports['qr-core']:RandomInt(1) .. exports['qr-core']:RandomStr(2) .. exports['qr-core']:RandomInt(3) .. exports['qr-core']:RandomStr(4))
				...
				if Player.Functions.AddItem(itemData["name"], amount, false, info) then
					TriggerClientEvent('QRCore:Notify', source, 9, Lang:t("success.yougave", {amount = amount, item = itemData["name"], name = GetPlayerName(tonumber(args[1]))}), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
					TriggerClientEvent('inventory:client:ItemBox', tonumber(args[1]), sharedItems[itemData["name"]], 'add')
				else
					TriggerClientEvent('QRCore:Notify', source, 9,  Lang:t("error.cant_give"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
				end
			else
				TriggerClientEvent('QRCore:Notify', source, 9,  Lang:t("error.not_exist"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
			end
		else
			TriggerClientEvent('QRCore:Notify', source, 9,  Lang:t("error.invalid_amount"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
		end
	else
...
exports['qr-core']:AddCommand("randomitems", "Give Random Items (God Only)", {}, false, function(source, args)
	local Player = exports['qr-core']:GetPlayer(source)
	local filteredItems = {}
...
exports['qr-core']:CreateUseableItem("snowball", function(source, item)
	local Player = exports['qr-core']:GetPlayer(source)
	local itemData = Player.Functions.GetItemBySlot(item.slot)
...
exports['qr-core']:CreateUseableItem("driver_license", function(source, item)
...
exports['qr-core']:CreateUseableItem("id_card", function(source, item)
	local PlayerPed = GetPlayerPed(source)
	local PlayerCoords = GetEntityCoords(PlayerPed)
	for k, v in pairs(exports['qr-core']:GetPlayers()) do
		local TargetPed = GetPlayerPed(v)
